...
安全状态将可恢复告警与锁存硬故障分开：START 发送/ACK 失败和持续视觉丢失使用
普通 STOP、撤销 STEP 后回到 IDLE；底盘链路看门狗已在底盘端停止双轮，STM32 仅
记录 WARN 并继续静态持球；限位、DIAG、持续编码器失效、传感器不一致和底盘硬故障
才进入锁存 FAULT。OLED 独立显示 `WARN` 与 `FAULT` 掩码，恢复路径不会要求用户为
一次短暂丢帧执行硬故障清除。

事件分级由无 BSP 依赖的 `app_safety_policy.c` 统一完成，策略仅有 WARN、受控停止和
锁存急停三种动作，并由主机测试固定。底盘看门狗告警首次出现时只发送一次 STOP
用于清除底盘端锁存，后续 1 kHz 检查不重复发送。
...
- TMC2209 复位默认禁用；视觉/反馈超时、限位、DIAG、底盘故障状态或传感器不一致时
  在一个控制周期内停止新 STEP 脉冲并拉高 ENN。单独的底盘 IMU 超时只撤销加速度
  前馈；底盘自身的远程会话看门狗负责停止双轮。
