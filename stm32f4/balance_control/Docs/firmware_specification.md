...
START_KEY 稳定按下 20 ms 后只产生一次边沿事件。主控发送 START 后每 300 ms 最多重试
3 次，确认必须同时匹配序号和运行状态；仍无 ACK 时停止重发但保持上层稳球。
底盘 250 ms 合法帧看门狗只置 `0x8000` 告警，不停止已在运行的本地循迹。下位机 IMU
超过 50 ms 未更新时，加速度前馈
权重平滑衰减为零；超过 200 ms 进入通信降级。

运行中底盘链路异常、START ACK 丢失、视觉预测、电位器接管和传感器不一致只置
WARN，不改变 RUNNING。视觉前 200 ms 继续有界预测，之后外环平滑将摆杆拉回水平；
总丢失约 600 ms 后才认定完全不可救。ABZ/PWM 不可用时，已标定电位器直接提供角度，
差分角速度经一阶低通后给内环。只有限位、DIAG、所有角度源失效或长时间球位不可观测
才锁存 FAULT；FAULT 仍只能断电重启。OLED 同时显示 WARN 和 FAULT。
...
- TMC2209 复位默认禁用；限位、DIAG、所有角度反馈失效或长时间球位不可观测时，
  在一个控制周期内锁存急停。可救降级恢复后必须无跳变地重新进入主闭环。
- ABZ 与电位器换算角度在标定工作区内保持允许误差；不一致持续超限时保留主编码器闭环并告警，而不是平均掩盖错误。
