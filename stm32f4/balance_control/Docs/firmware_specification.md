...
| 下位机 | USART2 | PD5/PD6 | 921600 bit/s，接收 IMU 加速度并发送启动/停止命令 |
...
| 两端限位 | EXTI5/6 | PE5/PE6 | 低有效常闭开关优先，断线等价于触发 |
...
START_KEY 稳定按下 20 ms 后只产生一次边沿事件；主控发送启动命令并等待状态确认，不把按键电平连续刷到串口。下位机 IMU 超过 50 ms 未更新时，加速度前馈权重平滑衰减为零；超过 200 ms 进入通信降级。
...
- 步进脉冲更新：1 kHz；脉冲输出由 TIM8 硬件产生，软件只更新 ARR/CCR/DIR。
- 角度/速度内环：500–1000 Hz，目标带宽先设 20–40 Hz，并根据机构共振降低。
- 位置外环：50–100 Hz，带宽保持为内环的 1/5 至 1/10。
...
- TIM6 控制周期抖动不超过周期的 5%，任何串口持续满速接收时也不丢失控制周期。
- TMC2209 复位默认禁用；通信超时、限位、DIAG 或传感器不一致时在一个控制周期内停止新 STEP 脉冲并拉高 ENN。
