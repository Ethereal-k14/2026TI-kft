...
安全机制只有一条故障路径：START 发送/ACK 失败、持续视觉丢失、持续编码器失效、
传感器不一致、底盘非健康、限位和 DIAG 均发送 ESTOP、禁用 STEP 并锁存 FAULT。
故障条件后续恢复也不改变状态，不存在软件清除接口。OLED 显示故障码和 `FAULT->POWER OFF`。
底盘 IMU 只是可选前馈，超时时权重平滑归零，不属于主控反馈故障。
...
- TMC2209 复位默认禁用；任一经过持续时间确认的视觉/摆杆/底盘故障，以及限位或
  DIAG，均在一个控制周期内锁存急停。故障后按键和链路恢复均不能重新使能执行器。
