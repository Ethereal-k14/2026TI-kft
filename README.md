...
│   └── to_kmodel.py        # nncase ONNX→.kmodel（Windows原生 / Linux / WSL2 均可）
...
2. 用 `tools/to_kmodel.py` + `nncase` 做 PTQ 量化，生成 `.kmodel`。支持 **Windows 原生**（已预置 .NET 8 向前兼容）或 **Linux/WSL2/Docker**（见 `docs/k230_deploy.md`）。
