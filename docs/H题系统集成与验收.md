...
| 远程启动会话链路超过 250 ms | MSPM0 看门狗停止底盘；STM32 将前馈归零并可继续保持球 | 底盘按旧巡线目标继续运动 |
| 视觉短时低置信度 | 在限定预测窗内仅用模型预测并收紧输出 | 用低置信度位置直接校正状态 |
| 视觉超过 200 ms | STEP 停止、ENN 禁用、等待重新建锁 | 用磁编码器/电位器冒充球位置 |
...
且要求 `last_command_seq` 匹配的 `state=running` 确认；底盘上报任意非零故障时执行
一次全系统急停，避免重复故障
检查持续刷串口。STOP/ESTOP 会解除该远程会话，重新启动必须再次满足全部前置条件。
