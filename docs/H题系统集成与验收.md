...
3. STM32 收到底盘心跳和新鲜 IMU、视觉锁定且限位正常后才接受启动按键；启动命令
   在 300 ms 内必须收到匹配 START 序号的运行状态确认，否则两端进入停止/故障，
   不能用旧状态帧假定本次命令已执行。
...
安全策略按后果而非按“是否异常”分级：硬件可能继续伤害机构或反馈已不可控时才锁存
`FAULT`；能够确定性停住但可能自行恢复的条件执行受控停止并回到 `IDLE`；不影响
滚球主反馈的条件只撤销对应功能。OLED 分别显示 `WARN` 和 `FAULT`，不得用同一红灯
掩盖所有原因。

| 条件 | 允许的降级 | 必须禁止 |
|---|---|---|
| 底盘 IMU 超过 50 ms | 平衡加速度前馈平滑衰减，循迹暂时只用红外/编码器 | 继续使用陈旧加速度或瞬间跳变前馈 |
| 远程启动会话链路超过 250 ms | MSPM0 看门狗停止底盘；STM32 保持静止平台上的球、记录告警并只发送一次 STOP 确认 | 底盘按旧巡线目标继续运动，或无必要地立即释放球/重复刷 STOP |
...
| ABZ 异常但 PWM 已标定 | 限速进入维护降级并提示故障 | 比赛中无告警切换反馈源 |
| 任一常闭限位断线/触发 | 立即停止并锁存，人工确认后复位 | 自动反向脱困或自动清故障 |
| 红外丢线超过阈值 | 底盘立即停止并上报 | 按最后转向无限搜索 |
| 过流、TMC DIAG、任务栈/堆故障 | 抢占正常控制并关闭执行器 | 等待平滑规划器慢慢降速 |

当前硬故障集合为限位、TMC DIAG、持续编码器失效、ABZ/电位器持续不一致和底盘
上报的非看门狗故障。清除硬故障前重新检查物理输入、编码器、视觉、底盘健康和
传感器一致性；可恢复告警不需要故障清除，只在条件恢复并重新满足启动门槛后重启。
事件到动作的唯一映射位于硬件无关的 `app_safety_policy`，新增异常必须先选择
`WARN / CONTROLLED_STOP / LATCHED_ESTOP` 并补主机测试，禁止在调度器里直接创造
第四种停机语义。
...
可恢复降级，STM32 继续在静止平台上持球并以单次 STOP 清除底盘看门狗锁存；其余
非零底盘故障执行一次全系统急停，避免重复故障检查持续刷串口。STOP/ESTOP 会解除
...
应得到仓库目录、跨端协议常量、K230、循迹、IMU 刚体补偿、STM32 平衡算法和安全
策略测试 `PASS`，IOC/目录/ARM 语法检查 `passed`。
