...
3. STM32 收到底盘心跳和新鲜状态、视觉锁定且限位正常后才接受启动按键；启动命令
   在 300 ms 内必须收到匹配 START 序号的运行状态确认，否则锁存 `FAULT`，
   停止两端执行器并断电重启；不能用旧状态帧假定命令已执行。
...
赛场只保留“正常运行”和“锁存停机”两种结果。短时相机丢帧、红外跳变和传感器尖峰由
预测/持续时间过滤，不当作故障；一旦超过阈值，这一轮已经失去验收意义，STM32 立即发送
ESTOP、禁用 STEP 并锁存 `FAULT`。不提供软件清故障、自动恢复或再次启动，只能断电检查后重启。

| 条件 | 允许的降级 | 必须禁止 |
|---|---|---|
| 底盘 IMU 超过 50 ms | 平衡加速度前馈平滑衰减，循迹暂时只用红外/编码器 | 继续使用陈旧加速度或瞬间跳变前馈 |
| 底盘链路超时或底盘报故障 | MSPM0 停止双轮；STM32 发送 ESTOP、禁用 STEP 并锁存 | 底盘按旧目标继续运动，或故障后自动恢复 |
| START 发送/确认失败 | 停止两端执行器并锁存 FAULT | 沿用旧 ACK 进入运行态，或自动重试 |
| 视觉短时低置信度 | 在限定预测窗内仅用模型预测并收紧输出 | 用低置信度位置直接校正状态 |
| 视觉持续失效 | 禁用两端执行器并锁存 FAULT | 用磁编码器/电位器冒充球位置，或自动重启 |
| 编码器持续无效或与电位器持续不一致 | 禁用两端执行器并锁存 FAULT | 使用冲突反馈继闭环，或因短尖峰立即锁死 |
...
故障码仅用于断电前定位原因：限位、DIAG、传感器不一致、视觉丢失、编码器丢失、
底盘异常和 START 失败。所有故障均进入同一不可恢复停机路径，不存在“清除后继续这一轮”的分支。
...
终止本轮运行。MSPM0 先保护双轮，STM32 在确认底盘非健康后发送 ESTOP、
禁用滚球执行器并锁存 FAULT。STOP/ESTOP 会解除远程会话；正常 STOP 后可再次
满足前置条件启动，ESTOP 后只能断电重启。
...
2. **停机检查**：拔开任一常闭限位、触发 DIAG、遮挡相机超过持续阈值或拔掉底盘串口，
   均应锁存 FAULT；故障条件恢复后按键仍不能启动，必须断电重启。
