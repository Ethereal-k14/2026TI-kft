...
3. STM32 在视觉锁定、至少一路摆杆角度反馈可用且限位正常后接受启动。START
   每 300 ms 最多重试 3 次，确认必须同时匹配命令序号和 `state=running`。ACK 仍丢失时
   不重复启动底盘、不退出稳球闭环，仅保留 WARN，避免因回包丢失丢掉已经启动的本轮。
...
安全逻辑优先“保住本轮”，但不在真正的 FAULT 后伪恢复。可救异常保持 `RUNNING`，
以 WARN 记录当前降级；数据恢复后自动返回原闭环。只有限位、DIAG、视觉长时间完全不可观测，
或编码器/PWM/已标定电位器全部失效时才锁存 `FAULT`。进入 FAULT 后仍不提供软件清除，
需断电检查。

| 条件 | 允许的降级 | 必须禁止 |
|---|---|---|
| 底盘 IMU 超过 50 ms | 平衡加速度前馈平滑衰减，循迹暂时只用红外/编码器 | 继续使用陈旧加速度或瞬间跳变前馈 |
| 底盘链路超时 | MSPM0 依靠红外、轮编码器和 IMU 继续完成当前单圈；STM32 前馈平滑归零 | 因非必需通信丢失立即杀死本地闭环 |
| START 发送/确认失败 | 300 ms 间隔最多重试 3 次；仍无 ACK 时保留 WARN 并继续稳球 | 无限重发 START 导致底盘计时重置 |
| 视觉丢失不超过约 600 ms | 前 200 ms 有界模型预测，之后外环平滑回水平持杆；恢复帧重新拉回球 | 用低置信度位置硬校正，或短时丢帧就停车 |
| 视觉超过约 600 ms 仍完全不可观测 | 锁存 FAULT，避免球脱离 25 cm 槽 | 无限盲控 |
| ABZ/PWM 失效但电位器已标定 | 电位器角度和低通差分角速度接管内环 | 因单一角度源丢失立即停车 |
| 编码器与电位器持续不一致 | 保留高带宽编码器闭环并告警，记录实车数据 | 对两路直接平均或一次尖峰就锁死 |
| 所有摆杆角度源持续无效 | 锁存 FAULT | 没有内环反馈时继续发 STEP |
...
运行中 WARN 为底盘链路、START ACK、视觉预测、电位器角度接管和传感器不一致；
对应条件恢复后自动清除。FAULT 只保留为限位、DIAG、长时间球位置不可观测和全部角度反馈失效；
所有 FAULT 均进入同一不可恢复停机路径。
...
远程 START 建立一个底盘会话；MSPM0 在 250 ms 内未再收到合法帧时以 `0x8000`
标记链路中断，但不撤销已运行的本地循迹。底盘仍受 300 ms 红外持续丢线、35 s 最大运行时间
和单圈终点判定保护。任意合法帧恢复时自动清除 `0x8000`，不重置单圈状态。
...
2. **救场检查**：运行中分别断开底盘串口、遮挡相机 300 ms、中断 ABZ/PWM，应依次看到
   底盘继续循迹且前馈归零、视觉恢复后重新拉回、已标定电位器接管内环。再遮挡超过
   约 600 ms 或断开全部角度源，必须锁存 FAULT；限位和 DIAG 仍立即急停。
