...
`SAFE / PRECISION / BALANCED / FAST` 四档均包含误差滤波、转弯降速、速度斜坡、
转向限幅、丢线搜索和 IMU 偏航阻尼。量产默认档由
...
7. 测量 STEP 正方向是否使球朝正坐标加速；符号不一致只反转 DIR 定义。先用
   `SAFE`，确认内环稳定后再闭合位置外环。
8. 底盘静止采集 IMU 零偏；做前后加速检查 `ax` 符号和时间戳。前馈增益从 0 开始，
   只有在符号、延迟和比例都确认后开启。
...
应得到三个算法测试 `PASS`、IOC/目录/ARM 语法检查 `passed` 且无空白错误。
