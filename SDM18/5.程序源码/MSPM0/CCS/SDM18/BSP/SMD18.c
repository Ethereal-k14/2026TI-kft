...
{
	delay_ms(200);//等待串口初始化完成	Wait for serial port initialization to complete
	
	stop_scan();//停止扫描	Stop Scanning
	delay_ms(200); //发送命令之间要有间隔	There should be a gap between sending commands
...
//	send_buf[8] = 0x6E;
	...
	USART1_Send_ArrayU8(send_buf,9);
...
void SMD18_setbaudrate(SDM18_Baud_t i)
{
	...
	USART1_Send_ArrayU8(send_buf,10);
	memset(send_buf,0,sizeof(send_buf));
...
void SDM18_Decode(uint8_t RxData)
{
	...
	static uint16_t len = 0;//数据长度 一般是14	The data length is usually 14
	...
	switch(step_index)
	{
		...
		case 1: Rx_buffer_temp[1] = RxData;//0x03
						...
						{
							step_index = 0;//数据不对,重置接收	Data is incorrect, reset receiving
						}
						break; 
		...
		case 6: Rx_buffer_temp[6] = RxData;  step_index++; //数据长度 LOW
						len = (Rx_buffer_temp[5]<<8 |Rx_buffer_temp[6]) -1;   //从0开始算	Counting from 0
						break; 
		case 7: if(index< len) 
						{
						...
						else //超出数据长度	Data length exceeded
						{
							step_index++;
							index = 0;//数据段步长归0	The data segment step length is reset to 0
						}
						break;
						
		//CRC校验码判断	CRC check code judgment
		...
		case 10: 
			...
			memset(Rx_buffer_temp,0,sizeof(Rx_buffer_temp));//清空	Clear
			...
			len = 0;
			step_index = 0;
			newlines = 1; //完整的数据	Complete data
			break;					
